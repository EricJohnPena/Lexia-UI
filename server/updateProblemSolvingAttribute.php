...
try {
    ...
    error_log("Received parameters: student_id=$student_id, module_number=$module_number, game_mode_id=$game_mode_id, subject_id=$subject_id, problem_solving=$problem_solving");

    if ($student_id === 0 || $module_number === 0 || $game_mode_id === 0 || $subject_id === 0 || $problem_solving < 0) {
    ...
    if ($checkStmt->num_rows > 0) {
        ...
        $updateStmt->bind_param("iiiii", $problem_solving, $student_id, $module_number, $game_mode_id, $subject_id);
        ...
        $insertQuery = "
            INSERT INTO students_progress_tbl (student_id, module_number, fk_game_mode_id, fk_subject_id, problem_solving_skills)
            VALUES (?, ?, ?, ?, ?)
        ";
        $insertStmt = $conn->prepare($insertQuery);
        $insertStmt->bind_param("iiiii", $student_id, $module_number, $game_mode_id, $subject_id, $problem_solving);
